...
### Popup Menu / Dropdown / Context Menu

All popup menus, dropdowns, context menus, and action menus **must** use the base `DropdownMenu` component from `src/components/ui/dropdown-menu.tsx`. No custom popup implementations are allowed.

**Container**
- Background: `--surface-container-high` (elevated surface)
- Border: `1px solid var(--outline-variant)`
- Radius: 12px (Panel)
- Shadow: Elevation Level 3 (`rgba(0,0,0,0.2) 0px 0px 0px 1px`)
- Min width: `8rem` (128px)
- Padding: 4px (internal `p-1`)
- Max height: constrained to viewport with overflow scroll when needed

**Menu Items** (`DropdownMenuItem`, `CheckboxItem`, `RadioItem`, `SubTrigger`)
- Font: `text-body-sm` — 13px (0.8125rem), weight 400, letter-spacing -0.13px
- Color: inherits `--popover-foreground`
- Padding: `px-2 py-1.5` (8px horizontal, 6px vertical)
- Radius: standard (4px) on focus/hover
- Hover/focus: `--accent` background, `--accent-foreground` text
- Disabled: `opacity-38`, no pointer events
- Icons: `--muted-foreground`, 16px (`size-4`), left-aligned with `gap-2`
- Destructive variant: `--destructive` text color, `--destructive/10` focus background

**Section Labels** (`DropdownMenuLabel`)
- Font: `text-label-sm` — 11px (0.6875rem), weight 510
- Transform: `uppercase`, `tracking-wider`
- Color: `--muted-foreground`
- Padding: `px-2 py-1.5`
- Purpose: non-interactive category headers that group related items
- Must be visually smaller and lighter than menu items to establish hierarchy

**Separators** (`DropdownMenuSeparator`)
- Border: `1px solid var(--border)`
- Margin: `-mx-1 my-1` (bleed to edges, 4px vertical spacing)

**Submenus** (`DropdownMenuSub` + `SubTrigger` + `SubContent`)
- SubTrigger: same styling as menu items, with a chevron-right indicator
- SubContent: same container styling as the parent menu
- Opens to the side (right by default, left if constrained)

**Radio & Checkbox Items**
- Indicator: left-aligned in an 8px-wide column (`pl-8` padding accounts for indicator space)
- Radio dot: `CircleIcon` 8px filled with `currentColor`
- Checkbox mark: `CheckIcon` 16px
- Selected state: standard item styling (no background change — indicator is sufficient)

**Keyboard Shortcut Labels** (`DropdownMenuShortcut`)
- Font: `text-label-sm` — 11px, weight 510
- Color: `--muted-foreground`
- Alignment: pushed to the right via `ml-auto`
- Tracking: `tracking-widest`

**Hierarchy Rules**
- Section labels → items → separator → next section (max 2 nesting levels)
- Top sections = visual/display settings (density, columns)
- Middle sections = data manipulation (sort, filter, pagination)
- Bottom sections = system actions (export, reset)
- Frequent actions within 1–2 clicks; avoid deep nesting

**Anti-patterns**
- Never build a custom popup/dropdown — always use `DropdownMenu` from `src/components/ui/dropdown-menu.tsx`
- Never use `text-body-md` (15px) or larger for menu items — menus are compact UI
- Never use the same font size for section labels and items — labels must be smaller
- Never omit section labels when a menu has more than one logical group
- Never use solid colored backgrounds for item hover — use `--accent` (translucent surface)
